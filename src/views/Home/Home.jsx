...
const Home = () => {
  ...
  const frees = games.length >= 12 ? games.slice(0, 12) : games.filter(game => game.is_free === true);
  const type = games.length >= 12 ? games.slice(0, 12) : games.filter(game => game.type === "demo" || game.type === "dlc");
  const controllerSupport = games.length >= 12 ? games.slice(0, 12) : games.filter(game => game.controller_support === "full");
