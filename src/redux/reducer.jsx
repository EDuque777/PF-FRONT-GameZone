...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case act.FILTER_GENRES:
      ...
      const filteredSearchsssssss = state.search.filter(
        (game) =>
          game.genres &&
          game.genres.some((categor) => categor.description === genre)
      );
    ...
    case act.FILTER_PLATFORMS:
      ...
      if (platform === "windows") {
        filteredSearchssss = state.search.filter(
          (game) => game.platforms.windows === true
        );
      } else if (platform === "linux") {
        filteredSearchssss = state.search.filter(
          (game) => game.platforms.linux === true
        );
      } else if (platform === "mac") {
        filteredSearchssss = state.search.filter(
          (game) => game.platforms.mac === true
        );
      }
      return {
        ...state,
    ...
    case act.ORDER_BY:
      ...
      if (orderBy === "des") {
        sortedSearch.sort((a, b) => {
          if (!a.release_date.coming_soon && !b.release_date.coming_soon) {
            ...
            !a.release_date.coming_soon &&
            b.release_date.coming_soon
          ) {
            return -1;
          } else if (
            a.release_date.coming_soon &&
      ...
      const filteredSearch = sortedSearch.filter(
        (game) => !game.release_date.coming_soon
      );
