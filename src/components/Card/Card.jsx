...
const Card = (props) => {
  const { id, price, name, image, appid } = props;
  //console.log(price);
  ...
  const prices = Number.isFinite(formattedNumber) ? formattedNumber : 0;
  const handleAdd = () => {
      dispatch(act.addCart({ id, price: prices, name, image }));
  };

  const handleAddWhish = () => {
      dispatch(act.addWhishList({ id, price: prices, name, image }));
  };
  ...
  return (
    <li className={style.box} key={id || appid}>
      ...
      <h3 className={style.price}>$ {prices}</h3>
